...
fn main() {
    let a = ans(include_str!("../../input11.txt"));
    println!("ans: {a}");
}
...
fn ans(s: &str) -> isize {
    ...
    fn offsets(total: isize, occupied: HashSet<isize>) -> Vec<isize> {
    ...
    let xo = offsets(total_cols, occupied_cols);
    let yo = offsets(total_rows, occupied_rows);
