...
impl Star {
    fn gear_ratio(&self, numbers: &Vec<Number>) -> Option<i32> {
        let adjacent: Vec<_> = numbers
            .into_iter()
            .filter(|n| n.is_adjacent(self))
            .collect();

        if adjacent.len() == 2 {
            let [first, second] = <[&Number; 2]>::try_from(adjacent).ok().unwrap();
            Some(first.num * second.num)
        } else {
            None
        }
    }
...
impl Number {
    fn is_adjacent(&self, star: &Star) -> bool {
        self.positions.iter().any(|p| p.is_adjacent(&star.0))
    }
}
...
fn process(input: &str) -> i32 {
    ...
    for (y, line) in input.lines().enumerate() {
        ...
        while let Some((x, char)) = chars.next() {
            ...
            if char.is_numeric() {
                ...
                stars.push(Star(Position::new(x, y)));
            }
        }
    }
    ...
    stars
        .into_iter()
        .filter_map(|s| s.gear_ratio(&numbers))
        .sum()
}
